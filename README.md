...
Then start the app normally under your process manager or service supervisor.
...
The repo also includes two utility scripts for timetable data in the underscore-separated format:

- `scripts/import_timetable.py` imports a full timetable into a SQLite database from `stdin`
- `scripts/update_timetable.py` updates existing weekly sessions from a text file
