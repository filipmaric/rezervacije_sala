...
"""Test the student RADIUS backend using values from rezervacije.env."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Send a student RADIUS login request using rezervacije.env."
    )
    parser.add_argument(
        "--env-file",
    ...
    backend = env_value("STUDENT_AUTH_BACKEND", env_file_values, "mock").lower()
    server = env_value("STUDENT_RADIUS_SERVER", env_file_values)
    secret = env_value("STUDENT_RADIUS_SECRET", env_file_values)
    dictionary_path = env_value("STUDENT_RADIUS_DICTIONARY", env_file_values)

    missing = [
        name
        for name, value in (
            ("STUDENT_RADIUS_SERVER", server),
            ("STUDENT_RADIUS_SECRET", secret),
            ("STUDENT_RADIUS_DICTIONARY", dictionary_path),
        )
        if not value
    ]
    if missing:
    ...
    if backend != "radius":
        print(
            f"STUDENT_AUTH_BACKEND is set to {backend!r}, not 'radius'.",
            file=sys.stderr,
        )
        return 2

    password = args.password if args.password is not None else getpass.getpass("Student password: ")
